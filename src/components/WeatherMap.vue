...
async function fetchMunicipalities() {
  ...
  try {
    ...
    try {
      ...
      response = await axios.get('https://public.opendatasoft.com/api/records/1.0/search/', {
        params: {
      ...
      console.error('Polygon filter failed, trying with bounding box:', polygonError);
...
async function fetchWeatherData(coordinates: [number, number]): Promise<string> {
  try {
    ...
    const response = await axios.get('https://archive-api.open-meteo.com/v1/archive', {
      params: {
        latitude: lat,
        longitude: lon,
        start_date: `${selectedYear.value}-${String(selectedMonth.value).padStart(2, '0')}-01`,
        end_date: `${selectedYear.value}-${String(selectedMonth.value).padStart(2, '0')}-28`,
        daily: 'temperature_2m_max',
        timezone: 'Europe/Madrid'
      }
    });

    const temps = response.data.daily.temperature_2m_max;
    
    // Calculate maximum temperature
    const maxTemp = Math.max(...temps);
    
    // Count days over specific temperature thresholds
    const daysOver30 = temps.filter((temp: number) => temp >= 30).length;
    const daysOver35 = temps.filter((temp: number) => temp >= 35).length;
    const daysOver40 = temps.filter((temp: number) => temp >= 40).length;
  ...
  if (activePopup.value) {
    activePopup.value.setContent(`
      <div>
        <h3>${municipality.name}</h3>
        ${weatherData}
        <p>Month: ${months[selectedMonth.value - 1]} ${selectedYear.value}</p>
      </div>
    `);
  }
}
